...
export default function TimePreferences() {
  ...
  const handleSave = () => {
    const labAssistantPreference = {
      teacherId: 'LA1', // Replace with actual lab assistant ID
      timeSlots: preferences,
      labTypes: selectedLabTypes,
    };
    console.log('Saving preferences:', labAssistantPreference);
  };

  const toggleLabType = (labType) => {
    setSelectedLabTypes(prev =>
      prev.includes(labType)
        ? prev.filter(t => t !== labType)
        : [...prev, labType]
    );
  };
  ...
  return (
    <div className="p-6">
      <div className="flex justify-between items-center mb-6">
        <h1 className="text-2xl font-bold">Set Lab Time Preferences</h1>
        <button
      ...
      <div className="bg-white rounded-lg shadow-md p-6 mb-6">
        <h2 className="text-lg font-semibold mb-4">Lab Specializations</h2>
          ...
          {LAB_TYPES.map((labType) => (
            <button
              ...
              className={`px-4 py-2 rounded-lg transition-colors ${
                selectedLabTypes.includes(labType)
      ...
      <div className="bg-white rounded-lg shadow-md p-6">
        ...
        {(preferences.length > 0 || selectedLabTypes.length > 0) && (
          <button
            onClick={handleSave}
